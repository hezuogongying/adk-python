...
from . import extract_page_data_agent_prompt # 导入提取页面数据 agent 的 prompt

# 定义提取页面数据 Agent
ExtractPageDataAgent = Agent(
    model=MODEL, # 指定使用的 LLM 模型
    name="extract_page_data_agent", # agent 名称
    description="从网页内容中提取重要数据", # agent 描述
    instruction=extract_page_data_agent_prompt.PROMPT, # agent 的指令（prompt）
